defmodule OllamaChat.MarkdownTest do
  ...
  describe "render_to_string/1" do
    test "renders basic markdown to HTML string" do
      html = Markdown.render_to_string("**bold** and *italic*")
      assert is_binary(html)
      assert html =~ "<strong>bold</strong>"
      assert html =~ "<em>italic</em>"
    end
    ...
    test "renders code blocks with syntax highlighting" do
      html = Markdown.render_to_string("```elixir\ndefmodule Foo do\nend\n```")
      assert html =~ "<pre"
      assert html =~ "<code"
      assert html =~ "language-elixir"
      # catppuccin_mocha inline styles should be present
    ...
    test "renders GFM tables" do
      ...
      html = Markdown.render_to_string(md)
      assert html =~ "<table>"
      assert html =~ "<th>"
      assert html =~ "<td>"
    end
    ...
    test "renders GFM strikethrough" do
      html = Markdown.render_to_string("~~deleted~~")
      assert html =~ "<del>deleted</del>"
    end

    test "renders autolinks" do
      html = Markdown.render_to_string("Visit https://example.com for more")
      assert html =~ "<a"
      assert html =~ "https://example.com"
    end

    test "renders headings" do
      html = Markdown.render_to_string("# Title\n## Subtitle")
      assert html =~ "<h1>Title</h1>"
      assert html =~ "<h2>Subtitle</h2>"
    end

    test "renders lists" do
      html = Markdown.render_to_string("- item 1\n- item 2")
      assert html =~ "<ul>"
      assert html =~ "<li>"
    end

    test "renders blockquotes" do
      html = Markdown.render_to_string("> quoted text")
      assert html =~ "<blockquote>"
    end

    test "renders inline code" do
      html = Markdown.render_to_string("Use `mix test` to run tests")
      assert html =~ "<code>"
      assert html =~ "mix test"
    end

    test "returns empty string for nil" do
      assert Markdown.render_to_string(nil) == ""
    end

    test "returns empty string for empty string" do
      assert Markdown.render_to_string("") == ""
    end

    test "handles plain text without markdown" do
      html = Markdown.render_to_string("Just plain text")
      assert html =~ "Just plain text"
    end

    test "always returns a binary string (not a tuple)" do
      html = Markdown.render_to_string("anything")
      assert is_binary(html)
      refute match?({:safe, _}, html)
    end
  end
end
